...
安装 Docker for Mac 后，打开 Docker 的首选项面板，转到 “资源”部分，并将分配的内存增加到 6GB。仅分配了 2GB 的 RAM 默认情况下，超集将无法启动。
Linux目录
按照 Docker 在 Linux 上安装 Docker。 说明适合您的 Linux 版本。因为未安装 Linux 上的基本 Docker 安装的一部分，一旦您有一个有效的引擎，请按照 Linux 的 docker-compose 安装说明进行操作。docker-compose
窗户
...
复制
或者，您也可以通过先签出来运行特定版本的超集 分支/标记，然后从变量开始。 例如，要运行 1.4.0 版本，请运行以下命令：docker-composeTAG
git checkout 1.4.0
...
2.	尝试连接到数据库时，请尝试使用 （Mac 用户、Ubuntu） 或 （Linux 用户） 作为主机名，而不是 。这是 docker 内部细节，正在发生的事情是，在 Mac 系统中，docker 为主机名创建一个 dns 条目，该条目解析为主机的正确地址，而在 linux 中并非如此（至少默认情况下）。如果这两个主机名都不起作用，那么您可能需要找到要使用的确切主机名，为此您可以这样做或查看必须由 docker 为您创建的接口的 IP 地址。或者，如果您甚至没有看到界面，请尝试（如果需要使用 sudo）并查看是否有条目并记下 IP 地址。localhosthost.docker.internal172.18.0.1host.docker.internalifconfigip addr showdocker0docker0docker network inspect bridge"Gateway"
